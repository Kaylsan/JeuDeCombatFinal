...
class Jeu:
	...
	def boucle_principale(self):
		...
		while self.jeu_encours:
			...
			if self.joueur.a_tire:
				if len(self.projectile_groupe) < self.joueur.tir_autorise and self.delta_temps > 0.5:
			...
			if self.joueur.vie <= 0:
				...
				self.joueur_vitesse_x = 0
				self.joueur.tir_autorise = 0
				self.joueur.a_sauter = False

			if self.enemie.vie <= 0:
				self.enemie.etat = 'mort'
				self.enemie.tir_autorise = 0
	...
	def recommencer(self):
		...
		self.joueur.etat = 'debout'
		self.enemie.etat = 'vivant'
		self.joueur.tir_autorise = 2
		self.enemie.tir_autorise = 2
